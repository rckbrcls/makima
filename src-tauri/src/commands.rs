...
#[tauri::command]
pub fn makima_state(state: State<'_, Arc<AppRuntime>>) -> DashboardState {
    state.data.lock().expect("state poisoned").clone()
}

#[tauri::command]
pub fn makima_add_repository(
...
#[tauri::command]
pub fn makima_repo_branches(path: String) -> Result<RepoBranches, String> {
...
#[tauri::command]
pub fn makima_delete_repository(
    state: State<'_, Arc<AppRuntime>>,
    repo: String,
) -> Result<(), String> {
    {
...
#[tauri::command]
pub fn makima_get_execution_logs(
    state: State<'_, Arc<AppRuntime>>,
    execution_id: u32,
