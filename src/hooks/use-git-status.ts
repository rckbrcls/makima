...
import { mapGitStatus, mapFileDiff } from '@/lib/code-types'
...
export function useGitStatus(options: UseGitStatusOptions = {}) {
  ...
  const fetchStatus = useCallback(async (repoPath?: string): Promise<GitStatus | null> => {
    ...
    setError(null)
    try {
      const result = await invoke<GitStatusDb>('git_status', { path: targetPath })
  ...
  const fetchAllDiffs = useCallback(
    async (repoPath?: string): Promise<FileDiff[]> => {
      const targetPath = repoPath ?? pathRef.current
      if (!targetPath) return []

      try {
        const result = await invoke<FileDiffDb[]>('git_diff_all', {
          repoPath: targetPath,
        })
