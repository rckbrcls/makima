...
export function useConversations() {
  ...
  const loadConversation = useCallback(async (id: string): Promise<Conversation | null> => {
    try {
      const dbConv = await invoke<DbConversation | null>('db_get_conversation', { id })
  ...
  const createConversation = useCallback(async (title: string): Promise<Conversation | null> => {
    try {
      const dbConv = await invoke<DbConversation>('db_create_conversation', { title })
      const conversation = dbConversationToConversation(dbConv)
        ...
        setConversations((prev) =>
          prev.map((c) => {
            ...
            return {
              ...
              ...(updates.status !== undefined && { status: updates.status as ConversationStatus }),
              ...(updates.state !== undefined && { state: updates.state as ConversationState }),
              updatedAt: Date.now(),
            }
          }),
        )
  ...
  const setConversationsLocal = useCallback((updater: React.SetStateAction<Conversation[]>) => {
