...
export function computeExtendedStats(state: DashboardState): ExtendedStats {
  const { executionHistory, commands, repositories, runQueue, liveExecutions } =
    state;

  // Basic stats from history
  const totalRuns = executionHistory.length;
  const successes = executionHistory.filter(
    (i) => i.status === "success",
  ).length;
