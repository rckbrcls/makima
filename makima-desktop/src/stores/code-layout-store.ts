  ...
  getItem: async (name: string): Promise<string | null> => {
    ...
    try {
      if (!tauriCodeLayoutStorage.store) {
        tauriCodeLayoutStorage.store = await Store.load(
          "code-layout.json",
          { autoSave: true },
        )
      }
      const value =
        await tauriCodeLayoutStorage.store.get<string>(name)
      return value ?? null
    } catch (error) {
      console.warn(
...
export const useCodeLayoutStore = create<CodeLayoutStore>()(
  persist(
    (set) => ({
      ...
      setLastActiveRepositoryId: (id) =>
        set({ lastActiveRepositoryId: id }),
...
export const useFileListView = () =>
  useCodeLayoutStore((s) => s.fileListView)

export const useDiffView = () =>
  useCodeLayoutStore((s) => s.diffView)
